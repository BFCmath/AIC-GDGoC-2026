...
class Agent:
    ...
    def act(self, obs: dict) -> int:
        try:
            ...
            if self._enemy_future_bomb_risk(grid, my_pos, enemies, bombs) >= 12:
            ...
            for x in range(grid.shape[0]):
                for y in range(grid.shape[1]):
                    cell = int(grid[x, y])
                    if cell == 4:      # capacity
                        targets.append(((x, y), 56 if bombs_left <= 1 else 36))
                    elif cell == 3:    # radius
                        targets.append(((x, y), 48 if my_radius <= 2 else 24))

            for pos, count in self._box_spots(grid, bombs).items():
                boxes_at = self._count_boxes_in_blast(grid, pos, my_radius)
                targets.append((pos, 20 + boxes_at * 22 + count * 4))
    ...
    def _best_move_to_targets(self, grid, start, bombs, players, danger, targets, enemies):
        ...
        while q:
            ...
            if t > 0 and pos in target_value:
                ...
                score -= 0.30 * self._enemy_future_bomb_risk(grid, pos, enemies, bombs)
                if score > best_score:
                    best_score = score
                    best = first
            if t >= 12:
    ...
    def _fallback_score(self, grid, pos, bombs, enemies, players, danger):
        ...
        score -= 0.45 * self._enemy_future_bomb_risk(grid, pos, enemies, bombs)
        if self.turn > 180:
