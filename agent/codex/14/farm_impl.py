        ...
        try:
            ...
            if 5 in legal:
                ...
                if value >= 10 and self._can_escape_after_bomb(grid, my_pos, bombs, players, my_radius):
                    ...
                    if boxes >= 1 or hit_enemies or trap_score >= 4 or escape_space >= 1:
                        return 5
                # Bomb-count tie-breaker: only after mid/late game, only when no material target
                # is immediately available, and only with a proven non-narrow escape.
                if self.turn > 300 and value >= 6 and self._can_escape_after_bomb(grid, my_pos, bombs, players, my_radius):
                    if self._escape_space_after_bomb(grid, my_pos, bombs, players, my_radius) >= 2:
                        return 5
