...
class Agent:
    ...
    team_id = "ResearchHybridV51_HardFutureTrap"
    ...
    def act(self, obs: dict) -> int:
        ...
        # Opportunistic item pickup before bombing. The official timeout tie-break
        # uses items before bombs, and items also improve future farming power.
        item_moves = []
        for action in valid:
            if action == 0:
                continue
        ...
        # Late/near item chase. Keep early box economy intact, but after the
        # map opens, items decide many equal-box timeouts versus V7.
        item_targets = set()
        h, w = grid.shape
        for ix in range(h):
            for iy in range(w):
        ...
        if item_targets:
            ...
            if best is not None:
                ip, d = best
                if d <= 2 or (self.turn > 155 and boxes_now == 0 and d <= 6) or (self.turn > 230 and boxes_now <= 1 and d <= 5):
        ...
        if bombs_left > 0 and my_pos not in bomb_positions:
            ...
            if my_pos not in hyp_danger.get(1, set()):
                ...
                if can_escape:
                    ...
                    if allow_bomb:
                        # Greedy safe farming: V4/V7 gain most rating through
                        # timeout tie-break stats. If a bomb immediately hits a box
                        # and we have a verified escape, do not let pathing score
                        # postpone the bomb.
                        if boxes_here >= 1 and post_safe >= 3:
                            return 5
    ...
    def _score_action(self, grid, players, bombs, my_pos, action, blocked, danger,
        ...
        if self._enemy_bomb_threat(grid, npos, players, enemy_ids):
            ...
            if self._enemy_hyp_bomb_trap_risk(grid, npos, players, bombs, enemy_ids, future_blocked):
                if safe_count < 12 or min_danger <= 5:
                    return -10**10
                score -= 900.0
                if safe_count < 22:
