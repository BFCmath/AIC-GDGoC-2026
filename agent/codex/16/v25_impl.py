...
class Agent:
    ...
    team_id = "MetaV25_NearItemRecall"
    ...
    def act(self, obs: dict) -> int:
        ...
        # Root movement may leave a bomb we are standing on, but future planning
        # must still treat that bomb cell as blocked.  V25 discarded my_pos
        # globally, which made the planner believe it could step back through
        # the just-placed bomb and occasionally choose a self-trapping pocket.
        ...
        # Minimal anti-procrastination: only force movement when the current
        # cell will explode very soon. This avoids V16's self-bomb waiting bug
        # without turning every future blast into a panic escape.
        ...
        # Very narrow item recall: after boxes drop a nearby item, grab it if
        # doing so is provably safe and we are not already sitting on a strong
        # multi-box bomb tile.  This targets the items tie-break without
        # disturbing the main C13 box-race policy.
        if self.turn > 18:
            boxes_now_for_override = self._count_boxes_in_blast(grid, my_pos, my_radius) if bombs_left > 0 else 0
            if boxes_now_for_override < 2:
                item_override = self._near_item_override_action(grid, my_pos, valid, blocked_all, danger, bomb_positions, bombs_left, int(me[4]))
                if item_override is not None:
                    return int(item_override)
            ...
            if my_pos not in hyp_danger.get(1, set()):
                ...
                if can_escape:
                    ...
                    allow_bomb = (
                        ...
                        or (self.turn > 200 and post_safe >= 15 and post_frontier >= 2)
                    )
                    if self.agent_id != 0 and self.turn < 70:
                        allow_bomb = allow_bomb and (boxes_here >= 1 and post_safe >= 10 and post_frontier >= 2)
                    if allow_bomb:
                        ...
                        if boxes_here > 0:
                            ...
                            if boxes_here == 1 and post_safe >= 5:
                                bomb_score += 90.0
                        elif self.turn > 200 and post_safe >= 15 and post_frontier >= 2:
                            bomb_score += 95.0
                            if self.turn > 320:
                                bomb_score += 70.0
                        # Conservative late-game tie-break pressure. Only applies after
                        # the economy phase and only with a large escape component.
                        if self.turn > 245 and post_safe >= 15 and post_frontier >= 2:
                            bomb_score += 35.0
                        if self.turn > 335 and post_safe >= 13:
                            bomb_score += 20.0
    ...
    def _score_action(self, grid, players, bombs, my_pos, action, blocked, danger,
        ...
        # After this root action, every live bomb cell is blocked, including a
        # bomb under our current feet once we leave it.
        ...
        score += 14.0 * safe_count + 7.0 * frontier_count
        if self.agent_id != 0 and self.turn < 70:
            score += 5.0 * safe_count + 3.0 * frontier_count
        ...
        if self._enemy_bomb_threat(grid, npos, players, enemy_ids):
            threat_limit = 15
            threat_penalty = 150.0
            if self.agent_id != 0 and self.turn < 120:
                threat_limit = 18
                threat_penalty = 260.0
            if safe_count < threat_limit or action == 0:
                score -= threat_penalty

            # Strong rule agents frequently place a bomb from their current cell
            # to close the only escape lane.  Static safe-count can overrate such
            # cells, so pessimistically test survival if the threatening enemy
            # drops a bomb this tick.
            if self._enemy_hyp_bomb_trap_risk(grid, npos, players, bombs, enemy_ids, future_blocked):
                score -= 620.0
                if min_danger <= 3:
                    score -= 360.0
                if safe_count < 22:
                    score -= 10.0 * (22 - safe_count)
    ...
    def _near_item_override_action(self, grid, my_pos, valid, blocked, danger, bomb_positions, bombs_left, bomb_bonus):
        best = None
        best_score = 0.0
        for action in valid:
            if action == 5:
                continue
            npos = self._next(my_pos, action)
            if not self._passable(grid, *npos) or (action != 0 and npos in blocked):
                continue
            if npos in danger.get(1, set()):
                continue
            future_danger = self._shift_danger(danger, 1)
            future_blocked = set(blocked) | set(bomb_positions)
            if not self._survives_from(grid, npos, future_blocked, future_danger, horizon=8):
                continue
            dist = self._distance_map(grid, npos, future_blocked, future_danger, max_depth=5)
            for cell_type, base in ((self.ITEM_CAPACITY, 500.0 if bombs_left <= 1 else 420.0),
                                    (self.ITEM_RADIUS, 410.0 if bomb_bonus <= 1 else 360.0)):
                for (x, y), d in dist.items():
                    if d > 4:
                        continue
                    if int(grid[x, y]) != cell_type:
                        continue
                    score = base / (d + 1)
                    if action != 0:
                        score += 8.0
                    # Keep this override narrow: require a clearly nearby item.
                    if d == 0:
                        score += 140.0
                    if score > best_score:
                        best_score = score
                        best = action
        return best if best_score >= 150.0 else None
    ...
    def _nearest_item_score(self, grid, pos, blocked, danger, bombs_left, bomb_bonus):
        ...
        for x in range(h):
            for y in range(w):
                ...
                if d is None:
                    continue
                base = 390.0 if cell == self.ITEM_CAPACITY else 350.0
    ...
    def _enemy_hyp_bomb_trap_risk(self, grid, pos, players, bombs, enemy_ids, blocked):
        """Return True if an enemy bomb placed now would make this cell a trap.

        This is a deliberately local pessimistic model: it only fires when the
        candidate cell is already in the blast line of an enemy who can place a
        bomb.  The extra bomb is treated as both future danger and a blocked
        escape cell after this root action.
        """
        for eid in enemy_ids:
            ...
            ep = (int(players[eid][0]), int(players[eid][1]))
            er = max(1, min(5, int(players[eid][4]) + 1))
            if pos not in self._blast_tiles(grid, ep[0], ep[1], er):
                continue
            # If the enemy is already standing on a live bomb, it cannot place
            # another there this tick according to engine rules.
            if any(int(b[0]) == ep[0] and int(b[1]) == ep[1] for b in bombs):
                continue
            hyp_bombs = self._with_hypothetical_bomb(bombs, ep, eid, timer=7, radius=er)
            hyp_danger, _ = self._danger_schedule(grid, hyp_bombs, players, horizon=8)
            future_hyp = self._shift_danger(hyp_danger, 1)
            hyp_blocked = set(blocked) | {ep}
            if not self._survives_from(grid, pos, hyp_blocked, future_hyp, horizon=7):
                return True
        return False
