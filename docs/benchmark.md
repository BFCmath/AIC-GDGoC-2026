...
## Clash Tournament: Codex 8 & Hybrid PPO 2 vs Baseline (Codex 7, Codex 4, Claude 2)

Chúng tôi đã chạy 2 giải đấu Clash (mỗi giải 20 trận) để kiểm tra khả năng của **Codex 8** và **Hybrid PPO Agent 2** trước dàn baseline mạnh nhất.

### Clash 1: Codex 8 (SeatAwareAssassinV20) vs Codex 7 vs Codex 4 vs Claude 2
| Tên Agent | File Path | Số Trận Thắng | Tỷ Lệ Thắng | Nhận Xét |
|---|---|---|---|---|
| **Codex 7** | `agent/codex/7.py` | **11** | **55.0%** | Tiếp tục giữ vị thế độc tôn với tỷ lệ thắng rất cao. |
| **Codex 4** | `agent/codex/4.py` | **5** | **25.0%** | Vẫn cực kỳ hung hãn. |
| **Codex 8** | `agent/codex/8.py` | **4** | **20.0%** | Mặc dù là phiên bản SeatAwareAssassinV20 mới với lối chơi né góc hẹp, nhưng không vượt qua được Codex 7 và Codex 4 trong các pha giao tranh tổng. |
| **Claude 2** | `agent/claude/2.py` | **0** | **0.0%** | Hoàn toàn lép vế trước sức ép của các bản Codex. |

### Clash 2: Hybrid PPO Agent 2 vs Codex 7 vs Codex 4 vs Claude 2
| Tên Agent | File Path | Số Trận Thắng | Tỷ Lệ Thắng | Nhận Xét |
|---|---|---|---|---|
| **Codex 7** | `agent/codex/7.py` | **10** | **50.0%** | Vẫn là agent mạnh nhất. |
| **Codex 4** | `agent/codex/4.py` | **9** | **45.0%** | Rất sít sao với Codex 7 trong trận này. |
| **Claude 2** | `agent/claude/2.py` | **1** | **5.0%** | May mắn sống sót được 1 trận đấu dài hơi. |
| **Hybrid PPO 2** | `agent/codex/hybrid_ppo_agent_2/agent.py` | **0** | **0.0%** | **Chưa hiệu quả**. Model RL HybridBCPPO_Shielded hiện tại bị loại từ rất sớm (Died: HybridBCPPO_Shielded thường xuất hiện ở các tập đầu). Điều này cho thấy network chưa được train đủ lâu (hiện tại mới chạy scale nhỏ) để học được chiến thuật endgame, hoặc do PPO policy can thiệp sai lầm vào fallback rule. Cần train lâu hơn trên Kaggle như kế hoạch. |

**Kết luận chung:** **Codex 7** và **Codex 4** hiện tại vẫn là các top-tier rule agents. Hướng đi sắp tới: Đưa bản Hybrid RL lên train dài hạn trên Kaggle theo `docs/dl_approach_4.md` (đã có script `train_bc_ppo.py`) để vượt qua mức trần của Rule-based agents.
