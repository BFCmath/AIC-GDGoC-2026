...
Benchmark 6 agents: rotate through 4-player groups, corners, and seeds.
  ...
  - 4 random seeds
  - 15 matches per (group, corner, seed) = 240 per group = 720 total
...
# 3 complementary groups of 4; each agent appears in exactly 2 groups
GROUPS = [
    [0, 1, 2, 3],   # 7, 8, 16, 15
    [0, 1, 4, 5],   # 7, 8, 13, claude2
    [2, 3, 4, 5],   # 16, 15, 13, claude2
]

NUM_SEEDS = 4
NUM_CORNERS = 4
MATCHES_PER_CONFIG = 2
MAX_STEPS = 500
BASE_SEED = 42
...
def run_single_match(args):
    ...
    print(f"  [Worker {os.getpid()}] Match {match_idx+1} started | seed={seed} | seats={agent_names}", flush=True)
        ...
        for seat in range(4):
            if int(obs["players"][seat][2]) == 1:
                try:
                    ...
                    print(f"    [Worker {os.getpid()}] Match {match_idx+1} Agent {seat} Error: {e}", flush=True)
                    actions.append(0)
            else:
                actions.append(0)
    ...
    print(f"  [Worker {os.getpid()}] Match {match_idx+1} finished | steps={step} | ranks={ranks} | time={t_elapsed:.1f}s", flush=True)
...
def run_benchmark():
    ...
    for spec in AGENT_SPECS:
        ...
        stem = Path(spec).stem
        label = f"{parent}/{stem}" if parent in ["codex", "claude"] else stem
        labels.append(label)
    ...
    for gid, group in enumerate(GROUPS):
        for corner in range(NUM_CORNERS):
            ...
            for seed_idx in range(NUM_SEEDS):
                ...
                for ep in range(MATCHES_PER_CONFIG):
                    tasks.append((abs_paths, seat_order, seed + ep * 3, MAX_STEPS, match_idx))
                    match_idx += 1

    total_matches = len(tasks)

    print("=" * 78)
    print(f"  6-Way Rotate Benchmark  —  {total_matches} matches in parallel (max_workers=24)")
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=24) as executor:
        ...
        for future in concurrent.futures.as_completed(futures):
            ...
            if completed % 50 == 0 or completed == total_matches:
                print(f"  Progress: {completed}/{total_matches} matches finished...", flush=True)
