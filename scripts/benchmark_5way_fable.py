"""
Benchmark 5 agents: rotate through 4-player groups, corners, and seeds.

  - 5 complementary 4-agent groups covering all 5 agents (each group excludes 1 agent)
  - 4 corner rotations per group
  - 4 random seeds
  - Total 5 * 4 * 4 = 80 matches
...
# 5 groups of 4; each group leaves out one agent
GROUPS = [
    [0, 1, 2, 3],   # Excludes fable/1
    [0, 1, 2, 4],   # Excludes claude/2
    [0, 1, 3, 4],   # Excludes codex/13
    [0, 2, 3, 4],   # Excludes codex/15
    [1, 2, 3, 4],   # Excludes codex/7
]
...
def run_benchmark():
    ...
    print("=" * 78)
    print(f"  5-Way Rotate Benchmark  —  {total_matches} matches in parallel (max_workers=4)")
