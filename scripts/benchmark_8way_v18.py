"""
Benchmark 8 agents: rotate through 4-player groups, corners, and seeds.

  - All 70 combinations of 4 agents from the 8-agent pool (8C4 = 70)
  - 4 random seeds (no seat/corner rotations)
  - Total 70 * 4 = 280 matches (each agent plays 140 matches)
...
AGENT_SPECS = [
    ...
    "agent/simple_rule_agent.py",
]

# All 70 combinations of 4 agents from the 8-agent pool (8C4 = 70)
GROUPS = [list(g) for g in itertools.combinations(range(8), 4)]
...
def run_benchmark():
    ...
    # Generate all match tasks (no corner rotations, only seeds)
    tasks = []
    match_idx = 0
    for gid, group in enumerate(GROUPS):
        for seed_idx in range(NUM_SEEDS):
            seat_order = group
            seed = BASE_SEED + gid * 100 + seed_idx * 7
    ...
    print("=" * 78)
    print(f"  Codex 18 8-Agent Pool Rotate Benchmark  —  {total_matches} matches in parallel (max_workers=4)")
    ...
    for gid, group in enumerate(GROUPS):
        for seed_idx in range(NUM_SEEDS):
            ...
            seat_order_for_ts = [(ratings[group[s]],) for s in range(4)]
            new_ratings = ts_env.rate(seat_order_for_ts, ranks=ranks)
            for s in range(4):
                ratings[group[s]] = new_ratings[s][0]

            winners = [group[s] for s in range(4) if ranks[s] == 0]
